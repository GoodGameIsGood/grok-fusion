...
Inside every mutating wave: write or extend a failing repository-native test for the wave's acceptance first, implement the minimum to pass, then refactor with tests green. A wave without a runnable test needs an explicitly accepted reason recorded in its summary.
